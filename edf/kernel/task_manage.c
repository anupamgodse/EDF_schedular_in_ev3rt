...
act_tsk(ID tskid)
{
    ...
    //increment invocations of current task
    p_tcb->invocations++;

    /*
     * if the task is user cyclic task then extract its 
     * period from the cyccb table
     */
    int temp = tnum_cyc - tnum_scyc;
    if(tskid > temp) {
        period = cyccb_table[tnum_scyc+tskid-temp-1].p_cycinib->cyctim;
    }

    //calculate absolute deadline of current job
    p_tcb->current_deadline = p_tcb->invocations * period;
