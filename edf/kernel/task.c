...
initialize_task(void)
{
	...
	for (j = 0; i < tnum_tsk; i++, j++) {
        ...
        //initialize variables related to edf
        p_tcb->invocations = 0;
        //p_tcb->interrupts = 0;
...
/*
 *  Insert in the task queue according to edf
 *
 */

void queue_insert_edf(QUEUE *p_queue, QUEUE *p_entry) {
    QUEUE *temp = p_queue->p_next;

    /*
     * in the priority queue of task find the correct place to insert
     * the tcb of task, go forward until deadline is greater than current task
     */
    while(temp != p_queue &&  ((TCB *)p_entry)->current_deadline >= ((TCB *)temp)->current_deadline) {
        temp = temp->p_next;
    }
...
make_runnable(TCB *p_tcb)
{
    ...
    //insert the currently released task according to edf rule
	queue_insert_edf(&(ready_queue[pri]), &(p_tcb->task_queue));

	//queue_insert_prev(&(ready_queue[pri]), &(p_tcb->task_queue));
	primap_set(pri);

    /*
     * if the processor was not running any task then make it run 
     * the curretly released task
     */
	if (p_schedtsk == (TCB *) NULL) {
		p_schedtsk = p_tcb;
		return(dspflg);
	}
    
    /* 
     * Decide if to interrupt the lower priority task according to static priority or 
     * edf(if same priorities) 
     */
	if (pri < p_schedtsk->priority || p_tcb->current_deadline < p_schedtsk->current_deadline) {
		p_schedtsk = p_tcb;
	    return(dspflg);
	}
	return(false);
